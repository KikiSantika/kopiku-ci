    ...
    <nav class="navbar navbar-expand-lg bg-white sticky-top">
      <div class="container position-relative py-2">
        ...
        <div class="collapse navbar-collapse" id="navbarNavAltMarkup">
          <div id="menu-data" class="navbar-nav mx-auto">
...
<header class="container pt-5">
    <h1 id = "hero-tagline" class="text-center"> Boost your day with <br> a cup of coffee </h1>
    <div class="d-flex justify-content-center">
         <img id = "hero-banner" src="#">
</div>
</header>
...
<section>
    <div> 
        <div class ="row">
            <div class="col-6"> 
                <img height="600px" src="#" id = "about-img">
            </div>
            <div class="col-6">
            <p id = "about-desc"></p>
...
<section id="our-coffee">
      <div class="container py-5">
        ...
        <div id = "coffee-data" class="row mb-5" style="--bs-gutter-x: 4rem">
          
          
          
        </div>
        <div class="text-center">
          <a class="btn btn-primary rounded-0 mt-3" onclick="moreCoffee(this)">ALL MENU</a>
        </div>
      </div>
    </section>
    ...
    <section id="instagram" style="background-color:#F3F4F6;">
      <div class="container py-5">
        <div id = "ig-data" class="row mb-5" style="--bs-gutter-x: 2rem; --bs-gutter-y: 2rem">
...
<!-- Location -->
            ...
            <div>
              ...
              <div class="text-muted d-flex gap-4 mt-4">
                <div>
                  ...
                  <div id = "location-opening-hours"></div>
              </div>
            </div>
          </div>
        </div>
 ...
 <!-- Gallery -->
 <section id="gallery">
      <div id = "gallery-data" class="row" style="--bs-gutter-x: 0rem;">
        
      </div>
    </section>

     <!-- Footer Banner -->
    <section style="background-color:#F3F4F6; background-image:url(<?php echo base_url('resourses/img/bg-footer1.png')?>); background-position: bottom center; background-repeat: no-repeat">
...
<script>
    ...
    $.get("https://core-kopiku.sawala.dev/api/instagrams?populate=*", function(data, status){
    console.log(data)
    let dataHtml = $("#ig-data").html();
    data.data.map((e)=>{
        dataHtml += `
        <div class="col-4">
          <div class="img-card">
    ...
    $("#ig-data").html(dataHtml)

    })
    </script>
<script>
$.get("https://core-kopiku.sawala.dev/api/about?populate=*", function(data, status){

$("#hero-tagline").html(data.data.attributes.tagline)
$("#hero-banner").attr('src',data.data.attributes.banner.data.attributes.url)
$("#about-desc").html(data.data.attributes.description)
...
//Instagram Title 
$("#instagram-username").text(data.data.attributes.instagram_username)
$("#order-text").text(data.data.attributes.order_text)
$("#location-title").text(data.data.attributes.location_title)
$("#location-address").text(data.data.attributes.address)
$("#location-opening-hours").text(data.data.attributes.opening_hours)
$("#copyright").text(data.data.attributes.copyright)
$("#footer-tagline").text(data.data.attributes.footer_tagline)
$("whatsapp-link").text(data.data.attributes.whatsapp_link)
$("#facebook-link").text(data.data.attributes.facebook_link)
$("#instagram-link").text(data.data.attributes.instagram_link)
$("#twitter-link").text(data.data.attributes.twitter_link)
$("#footer-image").attr('src',data.data.attributes.footer_image.data.attributes.url)


let dataMenu = '';

data.data.attributes.page_menu.map((e)=>{
  dataMenu += `<a class="nav-link" href="#">${e.title}</a>`
})
